...
Token *generate_separator_or_opreator(char *current, int *current_index, TokenType type){
  Token *token = malloc(sizeof(Token));
...
Token *lexer(FILE *file){
  ...
  while(current[current_index] != '\0'){
    ...
    if(current[current_index] == ';'){
      token = generate_separator_or_opreator(current, &current_index, SEPARATOR);
      ...
      token = generate_separator_or_opreator_or_opreator(current,&current_index,OPERATOR);
    }
